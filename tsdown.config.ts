/**
 * tsdown build for dsh-workbench-window: the host-half lib (lib/index.js and the
 * lib/invariant.js companion, ESM node) plus the browser client bundles
 ...
 *   the package-name id `dsh-workbench-window` (the client-modules compose keys on
 ...
 *   `dsh-external/dsh-workbench-window`.
 *
 * Both bundles replicate the official DSH client-bundle preset and are
 * compiled from the same src/client/index.tsx source — only the registered
 * id and the output file name differ, so they cannot drift:
...
/** Wire/type layers a client bundle may inline (mirror of the official list). */
...
function purityGatePlugin(): NonNullable<UserConfig['plugins']> {
  return {
    name: 'dsh-workbench-window-client-purity',
...
function makeCssPlugin(pluginId: string): NonNullable<UserConfig['plugins']> {
  return {
    name: 'dsh-workbench-window-css-inline',
...
export default [
  ...
  clientBundle('dsh-workbench-window', 'client.js'),
  clientBundle('dsh-external/dsh-workbench-window', 'client-registry.js'),
